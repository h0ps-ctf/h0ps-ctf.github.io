---
layout: post
title:  "MapleCTF 2022 EBCSIC"
---
Last weekend I played MapleCTF 2022 with ARESx. For this event we teamed up with RCX and played as RCX/ARESx.
...
There are two regions mapped `rwx`. The first region containts our shellcode, the second is empty. The binary does not have ASLR thefore the addresses of the first two regions are static won't be different on the challenge server.

Based on the memory layout and the available opcodes our exploitation plan is to find a way to write arbitrary values to memory (a `/bin/sh` shellcode) and then return to it.
...
Being able to get arbitrary values into `ebp` and writing them to an arbitrary memory location we can now simply write a `execve("/bin/sh", 0, 0)` shellcode somewhere and return to it.
